...
到目前為止，Copilot 只能處理你直接給它的內容：你用 `@` 引用的檔案、對話歷史，以及它自己的訓練資料。但如果它能主動查詢你的 GitHub 儲存庫、瀏覽你的專案檔案，或查找某個函式庫的最新文件呢？

這正是 MCP（模型情境協定）的用途。它是一種讓 Copilot 連接外部服務、取得即時真實資料的方式。Copilot 連接的每個服務都稱為「MCP 伺服器」。在本章中，你將設定幾個這樣的連線，並體驗它如何大幅提升 Copilot 的實用性。
...
把 MCP 伺服器想像成瀏覽器擴充功能。你的瀏覽器本身可以顯示網頁，但擴充功能能讓它連接更多服務：
...
沒有擴充功能，瀏覽器還是有用；但加上擴充功能，它就變得超強大。MCP 伺服器對 Copilot 來說也是如此。它們讓 Copilot 能連接真實、即時的資料來源，讀取你的 GitHub 問題、探索檔案系統、取得最新文件等等。

***MCP 伺服器讓 Copilot 連接外部世界：GitHub、儲存庫、文件等***

> 💡 **關鍵觀念**：沒有 MCP，Copilot 只能看到你用 `@` 明確分享的檔案。有了 MCP，它能主動探索你的專案、查詢 GitHub 儲存庫、查找文件，全自動完成。

---

<img src="assets/quick-start-mcp.png" alt="電源線連接並產生明亮電火花，周圍漂浮著代表 MCP 伺服器連線的科技圖示" width="800"/>

# 快速開始：30 秒體驗 MCP

## 立即體驗內建的 GitHub MCP 伺服器
在設定任何東西之前，先來看看 MCP 的實際效果。
...
如果 Copilot 回傳了真實的提交資料，你就親眼見證了 MCP 的威力。這就是 GitHub MCP 伺服器代表你連線到 GitHub。但 GitHub 只是*其中一個*伺服器。本章將教你如何新增更多（檔案系統存取、最新文件等），讓 Copilot 能做得更多。
...
> 💡 **只看到 GitHub 伺服器？** 這是正常的！如果你還沒新增其他 MCP 伺服器，目前只會列出 GitHub。下一節你就會學會如何新增。
...
<img src="assets/configuring-mcp-servers.png" alt="雙手調整專業音控台上的旋鈕與滑桿，象徵 MCP 伺服器設定" width="800"/>

你已經見識過 MCP 的威力，現在來設定更多伺服器。你有兩種方式可以新增伺服器：**從內建註冊表安裝**（最簡單——CLI 互動式引導），或**手動編輯設定檔**（更彈性）。如果不確定，建議先從註冊表方式開始。

---

## 從註冊表安裝 MCP 伺服器

CLI 內建 MCP 伺服器註冊表，讓你能用互動式引導安裝熱門伺服器——完全不需編輯 JSON。
...
Copilot 會開啟互動式選擇器，顯示可用的伺服器。選擇一個後，CLI 會引導你完成所需設定（API 金鑰、路徑等），並自動加入設定檔。

> 💡 **為什麼用註冊表？** 這是最簡單的入門方式——你不需要知道 npm 套件名稱、指令參數或 JSON 結構，CLI 會自動處理。

---

## MCP 設定檔

MCP 伺服器設定於 `~/.copilot/mcp-config.json`（使用者層級，適用所有專案）或 `.mcp.json`（專案層級，放在專案根目錄）。如果你用過上面的 `/mcp search`，CLI 已經自動建立或更新這個檔案，但了解格式有助於自訂。
...
*大多數 MCP 伺服器以 npm 套件發佈，並透過 `npx` 執行。*
...
- 字串請用雙引號 `"`（不要用單引號）
- 最後一項後面不能有逗號
- 檔案必須是合法 JSON（不確定時可用 [JSON 驗證工具](https://jsonlint.com/)）
...
GitHub MCP 伺服器為內建，無需額外設定。以下是你可以新增的其他伺服器。**可依興趣選擇，或依序練習。**
...
> 💡 **`.` 路徑**：`.` 代表「目前目錄」。Copilot 可存取你啟動時所在目錄的檔案。在 Codespace 中，這是你的工作區根目錄。你也可以改用絕對路徑（如 `/workspaces/copilot-cli-for-beginners`）。
...
Context7 讓 Copilot 能存取熱門框架與函式庫的最新官方文件。不再依賴可能過時的訓練資料，Copilot 會抓取真正的現行文件。
...
前述所有 MCP 伺服器（filesystem、Context7）都在本機執行。但 MCP 伺服器也能遠端執行，只要 Copilot CLI 指向一個 URL 即可，無需 `npx` 或 `python`，也不用安裝本地相依套件。

[Microsoft Learn MCP Server](https://github.com/microsoftdocs/mcp) 就是好例子。它讓 Copilot CLI 直接存取官方 Microsoft 文件（Azure、Microsoft Foundry 與其他 AI 主題、.NET、Microsoft 365 等），可搜尋文件、抓取完整頁面、查找官方程式碼範例，而不必只靠模型訓練資料。
...
- **microsoft-skill-creator**：產生 Microsoft 技術相關自訂技能的 meta-skill
...
Copilot CLI 內建 `web_fetch` 工具，可抓取任何 URL 的內容。這對於拉取 README、API 文件或發行說明很方便，無需離開終端機，也不需 MCP 伺服器。
...
想讓 Copilot 連接你自己的 API、資料庫或內部工具嗎？你可以用 Python 打造自訂 MCP 伺服器。這完全是選用功能，因為預設伺服器（GitHub、filesystem、Context7）已涵蓋大多數需求。

📖 詳細教學請見 [自訂 MCP 伺服器指南](mcp-custom-server.md)，以書籍應用程式為例完整說明。

📚 更多背景知識請參考 [MCP for Beginners 課程](https://github.com/microsoft/mcp-for-beginners)。
...
以下是同時啟用 filesystem 與 Context7 伺服器的 `mcp-config.json` 範例：
...
將此檔案儲存為 `~/.copilot/mcp-config.json`（全域設定），或放在專案根目錄 `.mcp.json`（專案專屬設定）。
...
GitHub MCP 伺服器是**內建**的。如果你已登入 Copilot（初次設定時已完成），它就能直接運作，無需額外設定！
...
- **本機 dev container（Docker）**：容器啟動後執行 `gh auth login`，然後重啟 Copilot。
...
> 💡 **在自己的 fork 上練習？** 如果你 fork 了本課程儲存庫，也可以嘗試建立問題、拉取請求等寫入操作。稍後的練習會實際操作。

> ⚠️ **沒看到結果？** GitHub MCP 作用於儲存庫的遠端（github.com），不只本地檔案。請確認你的 repo 有 remote：執行 `git remote -v` 檢查。
...
**Web Fetch**：用內建 `web_fetch` 工具拉取任何 URL 的內容：
...
這些工作流程展現為什麼開發者會說「用了就回不去了」。每個範例都在單一會話中結合多個 MCP 伺服器。

<img src="assets/issue-to-pr-workflow.png" alt="MCP 問題到 PR 工作流程 - 展示從取得 GitHub 問題到建立拉取請求的完整流程" width="800"/>

*完整 MCP 工作流程：GitHub MCP 取得 repo 資料、Filesystem MCP 找出程式碼、Context7 MCP 提供最佳實踐，Copilot 負責分析*

以下每個範例都是獨立的。**可依興趣選擇，或全部閱讀。**

| 我想看…… | 跳至 |
|---|---|
| 多個伺服器協作 | [多伺服器探索](#multi-server-exploration) |
| 一次會話完成從問題到 PR | [問題到 PR 工作流程](#issue-to-pr-workflow) |
| 快速檢查專案健康狀態 | [健康儀表板](#health-dashboard) |

<details>
<summary><strong>多伺服器探索</strong> - 在同一會話中結合 filesystem、GitHub、Context7</summary>
...
**成果**：程式碼探索 → 歷史回顧 → 最佳實踐查詢 → 改善建議。**全程只需一個終端機會話，三個 MCP 伺服器協作完成。**

</details>

<details>
<summary><strong>問題到 PR 工作流程</strong> - 不離開終端機，從 GitHub 問題到拉取請求</summary>
<a id="issue-to-pr-workflow"></a>

#### 問題到 PR 工作流程（在你自己的儲存庫）

這在你有寫入權限的 fork 或儲存庫上效果最佳：

> 💡 **現在無法實作也沒關係。** 如果你目前只有唯讀 clone，稍後作業會實際練習。現在先閱讀流程即可。
...
- 所有檔案都很精簡（小於 100 行）－結構良好！

```

**結果**：多個資料來源在幾秒內彙整。若手動操作，需執行 grep、計算行數、檢查 git log 並瀏覽測試檔案。輕鬆省下 15 分鐘以上的工作量。
...
<img src="../assets/practice.png" alt="溫馨桌面設置，螢幕顯示程式碼、檯燈、咖啡杯與耳機，準備好動手練習" width="800"/>

**🎉 你已經掌握了核心要點！** 你了解了 MCP，學會了如何設定伺服器，也看過實際的工作流程。現在輪到你親自動手試試看。

---

## ▶️ 自己動手試試看

現在換你上場！完成以下練習，實際操作 MCP 伺服器與書籍應用程式專案。
...
**預期結果**：Copilot 會顯示來自 GitHub 遠端的最近提交訊息與分支名稱。

> ⚠️ **在 Codespace 中嗎？** 這會自動運作，驗證資訊會被繼承。如果你是在本地複製的專案，請確認 `gh auth status` 顯示你已登入。
...
**自我檢查**：當你能解釋為什麼「查詢我的儲存庫提交歷史」比手動執行 `git log` 並將輸出貼到提示中更好時，你就真正理解 MCP 了。
...
練習在書籍應用程式專案中同時使用 MCP 伺服器。請在同一個 Copilot 會話中完成以下步驟：

1. **確認 MCP 正常運作**：執行 `/mcp show`，確認至少 GitHub 伺服器已啟用
2. **設定檔案系統 MCP**（如果尚未設定）：建立 `~/.copilot/mcp-config.json`，內容為檔案系統伺服器設定
3. **探索程式碼**：請 Copilot 使用檔案系統伺服器來：
...
4. **檢查儲存庫活動**：請 Copilot 使用 GitHub MCP 來：
...
**成功標準**：你能在同一個 Copilot 會話中無縫結合檔案系統與 GitHub MCP 的資料，並能解釋每個 MCP 伺服器在回應中貢獻了哪些資訊。
...
請使用上方 [完整設定檔](#complete-configuration-file) 區段的 JSON，儲存為 `~/.copilot/mcp-config.json`。
...
`test_books.py` 測試了：`add_book`、`mark_as_read`、`remove_book`、`get_unread_books` 和 `find_book_by_title`。像是 `load_books`、`save_books`、`list_books` 沒有直接測試。`book_app.py` 的 CLI 函式與 `utils.py` 的輔助函式則完全沒有測試。

**如果 MCP 沒有作用：** 編輯設定檔後請重新啟動 Copilot。

</details>

### 進階挑戰：打造自訂 MCP 伺服器

想更進一步嗎？請參考 [自訂 MCP 伺服器指南](mcp-custom-server.md)，用 Python 打造一個能連接任意 API 的 MCP 伺服器。
...
| 錯誤 | 現象 | 解法 |
|------|------|------|
| 不知道 GitHub MCP 是內建的 | 嘗試手動安裝／設定 | GitHub MCP 預設就有。直接試：「列出本儲存庫最近的提交」 |
| 設定檔位置錯誤 | 找不到或無法編輯 MCP 設定 | 使用者層級設定在 `~/.copilot/mcp-config.json`，專案層級則是專案根目錄的 `.mcp.json` |
| 設定檔 JSON 格式錯誤 | MCP 伺服器載入失敗 | 用 `/mcp show` 檢查設定，並驗證 JSON 語法 |
| 忘記驗證 MCP 伺服器 | 出現「驗證失敗」錯誤 | 有些 MCP 需要額外驗證。請檢查各伺服器需求 |
...
這會重新驗證你的 GitHub 帳號。如果問題持續，請確認你的 GitHub 帳號對該儲存庫有必要權限。
...
你可以用兩種方式管理 MCP 伺服器：**在聊天會話中用 slash 指令**，或直接在終端機用 **`copilot mcp` 指令**（不需進入聊天會話）。

### 方式一：slash 指令（在聊天會話中）

這些指令適用於你已經在 `copilot` 內時：
...
| `/mcp auth <server-name>` | 重新驗證需 OAuth 的 MCP 伺服器（例如切換帳號後） |

### 方式二：`copilot mcp` 指令（直接在終端機）

你也可以直接在終端機管理 MCP 伺服器，無需先啟動聊天會話：
...
> 💡 **什麼時候用哪一種？** 已在聊天會話中時用 `/mcp` slash 指令。想在啟動會話前快速檢查或更改伺服器設定時，直接用終端機的 `copilot mcp`。

本課程大多時候只需 `/mcp show`。隨著你管理的伺服器越來越多，其他指令會越來越有用。
...
4. **多伺服器工作流程** 可在同一會話中結合多個資料來源
5. **兩種伺服器管理方式**：聊天內用 `/mcp` slash 指令，或終端機用 `copilot mcp`
...
你已經擁有所有基礎：模式、情境、工作流程、Agent、Skill 與 MCP。是時候把它們全部結合起來了。

在 **[第 07 章：整合應用](../07-putting-it-together/README.md)**，你將學到：

- 如何結合 Agent、Skill 與 MCP，打造統一的工作流程
- 從想法到合併 PR 的完整功能開發流程
- 用 hook 自動化
- 團隊協作最佳實踐
