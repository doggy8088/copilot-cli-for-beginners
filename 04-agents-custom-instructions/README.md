...
在第 03 章中，你已經精通了基本工作流程：程式碼審查、重構、除錯、測試產生與 git 整合。這些讓你在 GitHub Copilot CLI 上效率大增。現在，讓我們再進一步。

到目前為止，你一直把 Copilot CLI 當作通用型助手。Agent 讓你能賦予它特定角色與內建標準，例如：強制型別註記與 PEP 8 的程式碼審查員，或是會撰寫 pytest 測試案例的測試助手。你將看到，同樣的提示詞，交給有明確指示的 agent 處理，結果會明顯更好。
...
Agent 的運作方式也一樣。與其用通用型 AI，不如用專注於特定任務、懂得正確流程的 agent。只需設定一次指令，日後需要該專業時就能重複使用：程式碼審查、測試、安全、文件等。

<img src="assets/hiring-specialists-analogy.png" alt="聘請專家類比——就像修房子會找專業工匠，AI agent 也專精於特定任務，如程式碼審查、測試、安全與文件" width="800" />
...
3. **理解核心概念：** Agent 就像找專家諮詢而不是問通才。「前端 agent」會自動專注於無障礙與元件模式，你不用每次都提醒，因為這些已經寫在 agent 指令裡了。


## 內建 Agent

**你在第 03 章開發流程已經用過一些內建 agent！**
<br>`/plan` 與 `/review` 其實就是內建 agent。現在你知道背後發生什麼事了。完整清單如下：

| Agent | 呼叫方式 | 功能說明 |
|-------|---------------|--------------|
| **Plan** | `/plan` 或 `Shift+Tab`（切換模式） | 在寫程式前產生逐步實作計畫 |
| **Code-review** | `/review` | 針對暫存/未暫存變更給予聚焦、可行的回饋 |
...
**內建 agent 實例** - Plan、Code-review、Explore 與 Task 的呼叫範例
...
那 Task Agent 呢？它在幕後管理與追蹤執行狀況，並以清楚明瞭的格式回報：
...
> 💡 **多輪子 agent（subagent）**：子 agent（由 agent 啟動的背景任務）支援後續訊息。在 agent 背景執行時，你可以開啟 `/tasks` 檢視並傳送後續指示。你不用等它跑完才能再下指令。就像你可以在助手工作途中拍他肩膀補充說明一樣。
...
你可以輕鬆自訂 agent，納入你的工作流程！定義一次，隨時指揮！
...
> 💡 **沒用過 YAML frontmatter？** 它是檔案頂部、用 `---` 包圍的一小段設定。YAML 就是 `key: value` 配對。其餘部分就是一般 Markdown。
...
> 💡 **必填與選填**：`description` 欄位必填。其他如 `name`、`tools`、`model` 則為選填。
...
**本專案已在 [.github/agents/](../.github/agents/) 資料夾內提供範例 agent 檔案**。你可以自行撰寫，或修改現有範例。
...
> 💡 **切換 agent**：隨時可用 `/agent` 或 `--agent` 切換到其他 agent。要回到標準 Copilot CLI，使用 `/agent` 並選擇**無 agent**。

> 💡 **Agent 模式只限當前會話**：你選的 agent 只影響目前這個會話。每次用 `/new`、`/clear` 或開新終端機時，Copilot 會回到預設模式——agent 選擇不會自動延續。這讓每次會話都能從乾淨狀態開始，有助於專注工作。

---

# 更深入運用 Agent

<img src="assets/creating-custom-agents.png" alt="機器人在工作台上被組裝，周圍有零件與工具，象徵自訂 agent 的打造" width="800"/>

> 💡 **本節為選讀。** 內建 agent（`/plan`、`/review`）已足夠應付多數工作流程。當你需要某種專業知識能一致應用於所有工作時，再考慮自訂 agent。

下列主題各自獨立。**挑你有興趣的看，不用一次全讀完。**
...
| 整理、命名與分享 agent | [agent 組織與分享](#organizing--sharing-agents) |
...
<details>
<summary><strong>專家 vs 通用：看得見的差異</strong> - 為什麼 agent 產生的結果比通用提示更好</summary>

## 專家 vs 通用：看得見的差異

這正是 agent 展現價值的地方。來看看差異：
...
**差異**：同一個提示，結果天差地遠。agent 自帶你可能會忘記要求的專業。
...
當 agent 設定好後，Copilot 也能在複雜任務中把它們當工具呼叫。如果你要求一個全端功能，Copilot 可能會自動分派部分工作給適合的專家 agent。
...
<details>
<summary><strong>agent 組織與分享</strong> - 命名、檔案位置、指令檔、團隊共用</summary>
...
把 agent 檔案放在 `.github/agents/`，就會被版本控制。推送到 repo，團隊每個人都自動取得。但 agent 只是 Copilot 會讀取的其中一種檔案。它也支援**指令檔**，這類檔案會自動套用到每個會話，無需手動 `/agent`。

換個角度：agent 是你隨時能召喚的專家，指令檔則是團隊永遠啟用的規則。

### 檔案該放哪裡

你已經知道兩個主要位置（見上方 [Agent 檔案放哪裡](#where-to-put-agent-files)）。用這張決策樹選擇：
...
<details>
<summary><strong>Copilot 專案設定</strong> - AGENTS.md、指令檔、/init 快速設定</summary>

## Copilot 專案設定

Agent 是你隨選的專家。**專案設定檔**則不同：Copilot 會在每次會話自動讀取，了解你的專案慣例、技術棧與規則。沒有人需要手動 `/agent`，所有協作者都會自動套用這些情境。
...
Copilot 會掃描你的專案並建立專屬指令檔。你可以再自行編輯。
...
> 🎯 **剛開始用？** 請用 `AGENTS.md` 寫專案指令。其他格式可視需求再探索。

### AGENTS.md

`AGENTS.md` 是推薦格式。它是[開放標準](https://agents.md/)，Copilot 與其他 AI 工具都能讀。放在 repo 根目錄，Copilot 會自動讀取。本專案的 [AGENTS.md](../AGENTS.md) 就是實際範例。

典型的 `AGENTS.md` 會描述你的專案情境、程式碼風格、安全需求與測試標準。可依我們的範例自行撰寫。

### 自訂指令檔（.instructions.md）

想要更細緻控管，可將指令拆成主題式檔案。每個檔案聚焦一個面向，自動套用：
...
> 💡 **注意**：指令檔支援任何語言。這裡用 Python 是配合本課程專案，你也能為 TypeScript、Go、Rust 等技術建立類似檔案。

#### 用 `applyTo` 限定指令作用範圍

預設情況下，指令檔會套用到每個會話。若只想限定某些檔案型別，可在 YAML frontmatter（檔案最上方 `---` 區塊）加上 `applyTo` 欄位：
...
有了 `applyTo: "**/*.py"`，Copilot 只會在你處理 Python 檔案時載入這份指令。Python 風格指令不會干擾 Dockerfile 或 SQL 查詢的對話。
...
> 💡 **小技巧**：用引號包住萬用字元（如 `"**/*.py"`），可確保各作業系統與 shell 都正確解析。

#### 用 `@` 匯入其他檔案

你可以在 `AGENTS.md` 或任何指令檔內用 `@檔案路徑` 語法引用其他檔案。Copilot 會自動展開並納入該檔案內容，讓主檔案保持精簡，細節分散存放：
...
當指令變多時很方便。可拆成多個聚焦檔案，再用單一 `AGENTS.md` 以 `@` 匯入。這種語法在 `.github/copilot-instructions.md` 與其他指令檔也適用。

> 💡 **小技巧**：用 `@` 匯入可讓多個指令檔共用同一份基礎檔案。例如可有一份 `@.github/instructions/shared-rules.md`，讓所有指令檔都能引用。

**尋找社群指令檔**：可瀏覽 [github/awesome-copilot](https://github.com/github/awesome-copilot)，裡面有 .NET、Angular、Azure、Python、Docker 等多種技術的現成指令檔。

### 關閉自訂指令

若你想讓 Copilot 忽略所有專案設定（方便除錯或比較行為）：
...
你已看過[最小 agent 格式](#-add-your-agents)。這裡是一個用到 `tools` 屬性的進階 agent。建立 `~/.copilot/agents/python-reviewer.agent.md`：
...
>
> ⚠️ **僅限 VS Code**：`model` 屬性（選擇 AI 模型）僅支援於 VS Code，不適用於 GitHub Copilot CLI。你可以為跨平台 agent 檔案加上這欄，Copilot CLI 會自動忽略。

### 更多 agent 範本

> 💡 **新手注意**：下方範例為模板。**請依你專案實際技術替換內容。** 重要的是 agent 的*結構*，不是範例裡的技術。
...
實作範例中建立了 `reviewer` 與 `documentor` 兩個 agent。現在請練習為另一個任務——強化書籍應用程式的資料驗證——建立並使用 agent：

1. 建立 3 個專為書籍應用程式設計的 agent 檔案（`.agent.md`），每個 agent 一個，放在 `.github/agents/` 目錄下
2. 你的 agent 包含：
   - **data-validator**：檢查 `data.json` 是否有遺漏或格式錯誤的資料（如作者為空、年份為 0、欄位缺漏）
...
**起手範本**：於 `.github/agents/` 目錄下為每個 agent 建立一個檔案：

`data-validator.agent.md`:
```markdown
---
description: 分析 JSON 資料檔案中遺漏或格式錯誤的項目
---

你會分析 JSON 資料檔案中遺漏或格式錯誤的項目。
...
```markdown
---
description: 撰寫清晰 Python 文件的技術寫作者
---

你是一位撰寫清晰 Python 文件的技術寫作者。
...
> 💡 **注意：** 你應該已經在本地複製的這個 repo 中有 `samples/book-app-project/data.json`。如果缺少，請從原始 repo 下載：
...
> @samples/book-app-project/data.json 檢查書籍資料中作者為空或年份無效的項目
```
...
你已經建立了可隨選呼叫的 agent。現在來試試另一種方式：**指令檔案**，讓 Copilot 在每次會話中自動讀取，無需 `/agent`。
...
| 錯誤 | 結果 | 修正方式 |
|------|------|----------|
| agent frontmatter 缺少 `description` | agent 無法載入或無法被發現 | YAML frontmatter 中務必包含 `description:` |
| agent 檔案位置錯誤 | 使用時找不到 agent | 請放在 `~/.copilot/agents/`（個人）或 `.github/agents/`（專案） |
| 使用 `.md` 而非 `.agent.md` | 檔案可能不被辨識為 agent | 檔名請用如 `python-reviewer.agent.md` |
| agent 提示內容過長 | 可能超過 30,000 字元限制 | agent 定義請聚焦重點；詳細指令請用 skill |
...
**自訂指令未載入** - 請在專案中執行 `/init` 以設定專案專屬指令：
...
2. **自訂 agent** 是以 `.agent.md` 檔案定義的專家角色
3. **優秀的 agent** 具備明確專業、標準與輸出格式
4. **多 agent 協作** 可結合專業解決複雜問題
5. **指令檔案**（`.instructions.md`）可自動套用團隊標準
6. **一致輸出** 來自明確定義的 agent 指令

> 📋 **快速參考**：完整指令與快捷鍵請參見 [GitHub Copilot CLI 指令參考](https://docs.github.com/en/copilot/reference/cli-command-reference)。

---

## ➡️ 下一步

Agent 會改變 *Copilot 處理與執行目標行動* 的方式。接下來你將學習 **skills** —— 它們則改變 Copilot *執行哪些步驟*。想知道 agent 與 skill 有何不同？第五章會直接說明。

在 **[第五章：Skills 系統](../05-skills/README.md)**，你將學到：

- 如何讓 skill 依據你的提示自動觸發（無需斜線指令）
- 安裝社群 skill
