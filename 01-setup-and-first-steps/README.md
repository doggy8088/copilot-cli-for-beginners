...
> **觀看 AI 即時找出錯誤、解釋難懂程式碼，並產生可執行腳本。接著學習三種不同的 GitHub Copilot CLI 使用方式。**

這一章就是魔法的起點！你將親身體驗為什麼開發者會形容 GitHub Copilot CLI 就像隨時可聯絡的資深工程師。你會看到 AI 在幾秒內找出安全性漏洞、用簡單易懂的英文解釋複雜程式碼，並立即產生可執行的腳本。然後你會學會三種互動模式（互動、規劃、程式化），讓你清楚知道每個任務該用哪一種。

> ⚠️ **先備條件**：請先完成 **[第 00 章：快速開始](../00-quick-start/README.md)**。你必須安裝並驗證 GitHub Copilot CLI，才能執行以下示範。
...
> ⏱️ **預估時間**：~45 分鐘（閱讀 15 分鐘 + 實作 30 分鐘）
...
在進入精彩的示範前，先從一些簡單的提示開始，這些你現在就能嘗試。**不需要任何程式碼庫！** 只要打開終端機並啟動 Copilot CLI：
...
> 📖 **閱讀範例說明**：以 `>` 開頭的行是你在 Copilot CLI 互動模式下輸入的提示。沒有 `>` 前綴的行則是你在終端機執行的 shell 指令。

> 💡 **關於範例輸出**：本課程中的範例輸出僅供參考。由於 Copilot CLI 每次回應都可能不同，你看到的內容在措辭、格式和細節上會有所差異。請著重於 *回傳資訊的類型*，而非完全一樣的文字。

### 示範 1：幾秒內完成程式碼審查

課程中有包含一些故意設計有程式碼品質問題的範例檔案。讓我們來審查其中一個：

```bash
# 如果你還沒下載課程範例，先 clone 一份
...
> 💡 **`@` 是什麼？** `@` 符號告訴 Copilot CLI 讀取一個檔案。你會在第 02 章學到詳細內容。現在只要照著輸入即可。
...
**重點**：專業的程式碼審查只需幾秒。人工審查則會花上……嗯……遠超過這個時間！
...
> 💡 **注意**：Copilot CLI 也有一種 **自動駕駛（Autopilot）** 模式，會自動執行任務而不等待你的輸入。這很強大，但需要授權完整權限，並會自動消耗進階請求。本課程聚焦於下方三種模式。等你熟悉基本操作後，我們會再介紹 Autopilot。

---

## 🧩 真實世界比喻：外出用餐

把使用 GitHub Copilot CLI 想像成外出用餐。從規劃行程到點餐，不同情境會用不同方式：

| 模式 | 用餐比喻 | 適用時機 |
|------|----------------|-------------|
| **規劃（Plan）** | 用 GPS 規劃前往餐廳的路線 | 複雜任務——先規劃路線、檢查停靠點、同意計畫，再出發 |
...
*根據任務選擇模式：規劃模式先規劃路線，互動模式適合來回討論，程式化模式適合一次性快速取得結果*
...
熟悉後可以嘗試：
- **程式化模式**（`copilot -p "<你的提示>"`）適合快速單次提問
- **規劃模式**（`/plan`）適合需要先詳細規劃再開始寫程式的情境
...
**最適合**：探索、反覆調整、多輪對話。就像和服務生對話，可以即時回饋、調整需求。
...
**關鍵洞見**：互動模式會保留情境。每則訊息都會延續前面的內容，就像真實對話一樣。
...
注意每個提示都建立在前一個答案之上。你是在進行一場對話，而不是每次都從頭開始。
...
**最適合**：需要先檢視並規劃執行步驟的複雜任務。就像出發前用 GPS 規劃路線。

規劃模式會協助你在寫程式前，先產生逐步計畫。使用 `/plan` 指令或按 **Shift+Tab** 切換到規劃模式：

> 💡 **小技巧**：**Shift+Tab** 可在模式間切換：互動 → 規劃 → 自動駕駛。任何時候都能在互動模式下按下切換。
...
**關鍵洞見**：規劃模式讓你能在寫程式前先檢查、調整執行步驟。計畫完成後，還能請 Copilot CLI 把計畫存成檔案，例如「Save this plan to `mark_as_read_plan.md`」會建立一個包含計畫細節的 markdown 檔。

> 💡 **想挑戰更複雜的嗎？** 試試：`/plan Add search and filter capabilities to the book app`。規劃模式可從簡單功能一路擴展到完整應用程式。

> 📚 **自動駕駛模式**：你可能注意到 Shift+Tab 會切到第三種模式「自動駕駛」。在自動駕駛模式下，Copilot 會自動執行整個計畫，不會在每個步驟詢問你——就像把任務交給同事並說「做完再告訴我」。典型流程是規劃 → 同意 → 自動駕駛，所以你必須先會寫計畫。請先熟悉互動與規劃模式，之後可參考 [官方文件](https://docs.github.com/copilot/concepts/agents/copilot-cli/autopilot)。
...
**關鍵洞見**：程式化模式會直接給你答案並結束。不會有對話，只有輸入 → 輸出。
...
```
> ⚠️ **關於 `--allow-all`**：這個旗標會跳過所有權限提示，讓 Copilot CLI 可直接讀取檔案、執行指令、存取網址。這對程式化模式（`-p`）很重要，因為沒有互動模式可供你確認。只在你自己撰寫的提示、且信任的目錄下使用 `--allow-all`。不要用在不信任的輸入或敏感目錄。
...
這些指令適用於互動模式。**先學這六個就夠了**——涵蓋日常 90% 的需求：
...
| `/plan` | 寫程式前先規劃 | 複雜功能 |
| `/research` | 用 GitHub 和網路來源做深入研究 | 寫程式前需要先調查主題時 |
| `/model` | 顯示或切換 AI 模型 | 想換 AI 模型時 |
| `/exit` | 結束對話 | 完成後 |

這樣就能順利開始！熟悉後可以再探索其他指令。
...
> 💡 上述必備指令已涵蓋大部分日常需求。這份參考適合你準備進一步探索時查閱。

### Agent 環境

| 指令 | 功能說明 |
|---------|--------------|
| `/init` | 初始化 Copilot 指令集到你的程式庫 |
| `/agent` | 瀏覽並選擇可用的 Agent |
| `/skills` | 管理技能，擴充功能 |
| `/mcp` | 管理 MCP 伺服器設定 |

> 💡 技能會在 [第 05 章](../05-skills/README.md) 詳細介紹。MCP 伺服器則在 [第 06 章](../06-mcp-servers/README.md) 介紹。
...
|---------|--------------|
| `/model` | 顯示或切換 AI 模型 |
| `/delegate` | 指派任務給 GitHub 上的 Copilot 程式碼 Agent（雲端 Agent） |
| `/fleet` | 將複雜任務拆分成平行子任務，加速完成 |
| `/tasks` | 檢視背景子代理與分離的 shell 工作階段 |
...
> ⚠️ **請小心使用**：`/allow-all` 會跳過確認提示。適合信任的專案，但不建議用於不信任的程式碼。

### 對話

| 指令 | 功能說明 |
|---------|--------------|
| `/resume` | 切換到其他對話（可指定 session ID） |
| `/rename` | 重新命名目前對話 |
| `/context` | 顯示 context window 的 token 使用量與視覺化 |
| `/usage` | 顯示對話使用統計與數據 |
| `/session` | 顯示對話資訊與工作區摘要 |
| `/compact` | 摘要對話內容，減少 context 使用量 |
...
Copilot CLI 支援多種 AI 模型，包括 OpenAI、Anthropic、Google 等。你能使用哪些模型取決於訂閱方案與地區。用 `/model` 查詢與切換：
...
> 💡 **小技巧**：有些模型會消耗更多「進階請求」。標記 **1x**（如 Claude Sonnet 4.5）的模型是很好的預設選擇，效能與效率兼具。倍率較高的模型會更快用完進階配額，請留給真正需要時再用。
...
`-p` 旗標可讓你直接在終端機執行 Copilot CLI，不需進入互動模式。從專案根目錄複製貼上下列腳本，批次審查書籍應用程式的所有 Python 檔案。
...
1. **互動挑戰**：啟動 `copilot` 並探索書籍應用程式。詢問 `@samples/book-app-project/books.py` 並連續要求三次改進建議。
2. **規劃模式挑戰**：執行 `/plan Add rating and review features to the book app`。仔細閱讀計畫內容。合理嗎？
...
前面的實作聚焦在審查與重構 `book_app.py`。現在請你用同樣的技巧練習另一個檔案 `utils.py`：
...
範例中用 `/plan` 實作搜尋功能、用 `-p` 批次審查。現在請你針對同一個新任務——為 `BookCollection` 類別新增 `list_by_year()` 方法——用三種模式都做一次：
...
| 錯誤 | 會發生什麼事 | 修正方式 |
|---------|--------------|-----|
| 輸入 `exit` 而非 `/exit` | Copilot CLI 會把 "exit" 當成提示詞，不是指令 | 斜線指令都要以 `/` 開頭 |
| 用 `-p` 進行多輪對話 | 每次 `-p` 都是獨立的，沒有前後文記憶 | 需要情境延續請用互動模式（`copilot`） |
| 忘記用引號包住含 `$` 或 `!` 的提示 | shell 會先處理特殊字元，Copilot CLI 收不到正確內容 | 用引號包住提示：`copilot -p "What does $HOME mean?"` |

### 疑難排解

**"Model not available"** - 你的訂閱可能不包含所有模型。用 `/model` 查詢可用選項。

**"Context too long"** - 你的對話已用滿 context window。用 `/clear` 重設，或開新對話。

**"Rate limit exceeded"** - 請稍後幾分鐘再試。批次作業可考慮用程式化模式並加延遲。
...
1. **互動模式** 適合探索與反覆調整——情境會自動延續。就像和記得你說過什麼的人對話。
2. **規劃模式** 適合較複雜的任務。先檢查計畫再執行。
3. **程式化模式** 適合自動化。不需互動。
4. **四個必備指令**（`/help`、`/clear`、`/plan`、`/exit`）就能涵蓋大部分日常需求。> 📋 **快速參考**：完整指令與捷徑清單請參見 [GitHub Copilot CLI 指令參考](https://docs.github.com/en/copilot/reference/cli-command-reference)。

---

## ➡️ 接下來

現在你已經了解三種模式，讓我們來學習如何讓 Copilot CLI 瞭解你的程式碼情境。
