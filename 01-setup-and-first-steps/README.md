...
> **觀看 AI 即時找出錯誤、解釋難懂的程式碼，並產生可運作的腳本。接著學習三種使用 GitHub Copilot CLI 的方式。**

這一章就是魔法的起點！你將親身體驗為什麼開發者形容 GitHub Copilot CLI 就像隨時能聯絡的資深工程師。你會看到 AI 在幾秒內找出安全性漏洞、用簡單英文解釋複雜程式碼，並即時產生可運作的腳本。然後你會學會三種互動模式（互動式、規劃、程式化），讓你知道任何任務該用哪一種。
...
在進入精彩示範之前，先從一些簡單的提示詞開始，現在就可以嘗試。**不需要程式碼倉庫**！只要打開終端機並啟動 Copilot CLI：
...
**關鍵洞察**：GitHub Copilot CLI 是對話式的。你不需要特殊語法，只要用簡單英文提問即可。

## 看看實際運作

現在來看看為什麼開發者稱這是「隨時能聯絡的資深工程師」。

> 📖 **閱讀範例說明**：以 `>` 開頭的行是你在 Copilot CLI 互動式對話中輸入的提示詞。沒有 `>` 前綴的行是你在終端機執行的 shell 指令。

> 💡 **關於範例輸出**：本課程中的範例輸出僅供參考。因 Copilot CLI 回應每次都不同，你的結果在措辭、格式和細節上會有所差異。請專注於回傳的*資訊類型*，而非完全相同的文字。

### 示範 1：秒級程式碼審查

課程提供了帶有刻意程式碼品質問題的範例檔案。如果你在本機操作且尚未複製 repo，請執行下方 `git clone` 指令，進入 `copilot-cli-for-beginners` 資料夾，然後啟動 Copilot。

```bash
# 若你在本機操作且尚未複製課程倉庫，請執行
...
進入 Copilot CLI 互動式對話後，執行下列提示：

```
> Review @samples/book-app-project/book_app.py for code quality issues and suggest improvements
```

> 💡 **`@` 符號是做什麼用的？** `@` 符號告訴 Copilot CLI 讀取檔案。你會在第 02 章學到詳細內容。現在只要照指令複製即可。
...
**結果**：（你的輸出會不同）Copilot CLI 讀取檔案、理解程式碼，並用簡單英文解釋。
...
**重點**：複雜程式碼被解釋得像耐心導師一樣清楚。

---

### 示範 3：產生可運作的程式碼

需要一個你可能要 Google 15 分鐘的函式？仍在對話中：
...
你剛剛看到 Copilot CLI 能做什麼。現在來了解*如何*有效運用這些能力。關鍵在於知道不同情境該用哪一種互動模式。

> 💡 **注意**：Copilot CLI 也有一種 **自動駕駛（Autopilot）**模式，可以自動執行任務而不等待你的輸入。這很強大，但需要授權完整權限，並會自動使用 premium 請求。本課程專注於下方三種模式。等你熟悉基礎後，我們會再介紹自動駕駛模式。
...
| 模式 | 用餐比喻 | 適用時機 |
|------|----------|----------|
| **規劃（Plan）** | GPS 導航到餐廳 | 複雜任務——規劃路線、檢查停靠點、同意計畫再出發 |
| **互動式（Interactive）** | 與服務生對話 | 探索與反覆調整——提問、客製化、即時回饋 |
...
熟悉後再嘗試：
- **程式化模式**（`copilot -p "<你的提示詞>"`）適合快速一次性問題
...
**最適合**：探索、反覆調整、多輪對話。就像和服務生對話，可以即時回饋並調整點餐。
...
如同前面示範，你會看到提示符號，可以自然輸入。想查詢可用指令，只要輸入：

```
> /help
```

**關鍵洞察**：互動式模式會保留情境。每則訊息都會建立在前一則之上，就像真實對話。
...
注意每個提示都建立在前一個答案之上。你是在對話，而不是每次都重頭開始。
...
**最適合**：複雜任務，需要先檢查方法再執行。就像出發前用 GPS 規劃路線。
...
> 💡 **提示**：**Shift+Tab** 可在模式間切換：互動式 → 規劃 → 自動駕駛。任何時候都可在互動式對話中按下切換，不需輸入指令。
...
**關鍵洞察**：規劃模式讓你在寫程式前先檢查並調整方法。計畫完成後，你甚至可以請 Copilot CLI 把計畫存到檔案，例如「Save this plan to `mark_as_read_plan.md`」會建立一個包含計畫細節的 markdown 檔案。

> 💡 **想要更複雜的功能？** 試試：`/plan Add search and filter capabilities to the book app`。規劃模式可從簡單功能擴展到完整應用程式。

> 📚 **自動駕駛模式**：你可能注意到 Shift+Tab 會切換到第三種模式——**自動駕駛（Autopilot）**。在自動駕駛模式下，Copilot 會自動執行整個計畫，不會在每一步等你輸入——就像把任務交給同事並說「完成後再通知我」。典型流程是規劃 → 同意 → 自動駕駛，所以你要先學會寫計畫。你也可以用 `copilot --autopilot` 直接啟動自動駕駛。建議先熟悉互動式與規劃模式，再參考 [官方文件](https://docs.github.com/copilot/concepts/agents/copilot-cli/autopilot)。
...
```
> ⚠️ **關於 `--allow-all`**：這個旗標會跳過所有權限提示，讓 Copilot CLI 讀取檔案、執行指令、存取網址而不需詢問。這對程式化模式（`-p`）很重要，因為沒有互動式對話可批准動作。只在你自己撰寫的提示詞和信任的目錄中使用 `--allow-all`。不要用於不信任的輸入或敏感目錄。
...
這些指令很適合剛開始學 Copilot CLI 時掌握：

| 指令 | 功能說明 | 適用時機 |
|------|----------|----------|
| `/ask` | 提問不會影響對話歷史 | 想要快速答案但不想干擾目前任務時 |
| `/clear` | 清除對話並重新開始 | 換主題時 |
| `/help` | 顯示所有可用指令 | 忘記指令時 |
| `/model` | 顯示或切換 AI 模型 | 想換 AI 模型時 |
| `/plan` | 寫程式前先規劃工作 | 複雜功能時 |
| `/research` | 用 GitHub 與網路來源做深入研究 | 寫程式前需要調查主題時 |
| `/exit` | 結束對話 | 完成時 |

> 💡 **`/ask` 與一般聊天的差異**：通常你發送的每則訊息都會成為對話的一部分，影響後續回應。`/ask` 是「不記錄」的捷徑——適合像 `/ask What does YAML mean?` 這種一次性問題，不會污染你的對話情境。
...
> 💡 上述必學指令涵蓋日常大部分需求。這份參考適合你準備探索更多時使用。
...
|------|----------|
| `/agent` | 瀏覽並選擇可用 Agent |
| `/env` | 顯示已載入的環境細節——哪些指令、MCP 伺服器、技能、Agent、外掛已啟用 |
| `/init` | 初始化 Copilot 指令至你的 repo |
| `/mcp` | 管理 MCP 伺服器設定 |
| `/skills` | 管理技能以增強能力 |
...
| `/fleet` | 將複雜任務分拆為平行子任務以加速完成 |
| `/model` | 顯示或切換 AI 模型 |
| `/tasks` | 檢視背景子 Agent 與分離 shell 對話 |
...
> ⚠️ **請小心使用**：`/allow-all` 與 `/yolo` 會跳過確認提示。適合信任的專案，但不信任的程式碼要小心。

### 對話

| 指令 | 功能說明 |
|------|----------|
| `/clear` | 放棄目前對話（不存歷史），重新開始 |
| `/compact` | 對話摘要以減少情境用量 |
| `/context` | 顯示情境窗口 token 用量與視覺化 |
| `/new` | 結束目前對話（存入歷史以便搜尋／繼續），重新開始 |
...
| 指令 | 功能說明 |
|------|----------|
| `/statusline`（或 `/footer`） | 客製底部狀態列顯示項目（目錄、分支、努力、情境窗口、配額） |
| `/theme` | 檢視或設定終端機主題 |
...
> 💡 **提示**：有些模型會消耗更多「premium 請求」。標註 **1x**（如 Claude Sonnet 4.5）是很好的預設選擇，既強大又高效。倍率更高的模型會更快用完 premium 配額，建議留給真正需要時再用。
...
用 `/plan` 讓 Copilot CLI 在寫程式前先規劃實作：
...
`-p` 旗標讓你直接在終端機執行 Copilot CLI，不需進入互動模式。從 repo 根目錄複製貼上下列腳本，批次審查書籍應用程式的所有 Python 檔案。
...
1. **互動式挑戰**：啟動 `copilot`，探索書籍應用程式。詢問 `@samples/book-app-project/books.py`，連續請求改善三次。
...
GitHub Copilot CLI 支援**遠端對話**，讓你可從網頁瀏覽器（桌機或手機）或 GitHub Mobile app 監控並互動正在執行的 CLI 對話，不需親自在終端機操作。
...
Copilot CLI 會顯示連結並提供 QR code。用手機或桌機瀏覽器開啟連結，即可即時觀看對話、發送後續提示、檢查計畫並遠端操控 Agent。對話是用戶專屬的，你只能存取自己的 Copilot CLI 對話。

你也可以在任何活躍對話中隨時啟用遠端存取：
...
前面實作主要聚焦於審查與重構 `book_app.py`。現在請你在另一個檔案 `utils.py` 練習同樣技巧：
...
**可嘗試的範例提示：**
```bash
> @samples/book-app-project/utils.py 這個檔案中的每個函式都做什麼？
> 為 get_user_choice() 增加驗證，讓它能處理空白輸入和非數字輸入
> 如果 get_book_details() 收到空字串作為書名會發生什麼事？請為此加入防呆機制。
> 為 get_book_details() 加上一份完整的 docstring，包含參數說明與回傳值
```
...
範例中已用 `/plan` 建立搜尋功能、用 `-p` 進行批次審查。現在請針對一個新任務同時嘗試三種模式：為 `BookCollection` 類別新增 `list_by_year()` 方法：

1. **互動模式**：`copilot` → 逐步請 Copilot 設計並實作該方法
...
| 在多輪對話中使用 `-p` | 每次 `-p` 執行都是獨立的，不會記住前一次內容 | 需要累積情境時請用互動模式（`copilot`） |
...
**"Context too long"** - 你的對話已用滿情境窗口。請用 `/clear` 重設，或開啟新 session。

**"Rate limit exceeded"** - 請稍候幾分鐘再試。若需大量操作可考慮用程式化模式並加入延遲。
...
在 **[第二章：情境與對話](../02-context-conversations/README.md)**，你將學到：

- 用 `@` 語法參照檔案與目錄
- 用 `--resume` 和 `--continue` 管理 session
- 為什麼情境管理讓 Copilot CLI 如虎添翼

---

**[← 回到課程首頁](../README.md)** | **[繼續前往第二章 →](../02-context-conversations/README.md)**
