...
讓 Copilot CLI 能夠在*所有會話*之間記住事實與偏好設定的功能，不僅限於單一會話。與會話歷史不同（只儲存特定對話），記憶會全域持續存在，並在未來會話自動套用。可透過 `/memory` slash 指令管理（`/memory on`、`/memory off`、`/memory show`）。記憶可限定於你的使用者帳號（所有儲存庫皆可見），或限定於特定儲存庫（與協作者共享）。
