...
> **如果 Copilot 能自動套用你們團隊的最佳實踐，而不用你每次都解釋一次，會怎樣？**

在本章中，你將學習 Agent 技能：Copilot 會在與你的任務相關時自動載入的指令資料夾。Agent 會改變 Copilot *思考的方式*，而技能則教 Copilot *完成任務的具體方法*。你將建立一個安全性稽核技能，讓 Copilot 在你詢問安全性時自動套用，打造團隊標準的審查規範，確保程式碼品質一致，並學會技能如何在 Copilot CLI、VS Code 及 GitHub Copilot 雲端 agent 中運作。
...
<img src="assets/how-skills-work.png" alt="發光的 RPG 風格技能圖示，透過光軌連結在星空背景上，代表 Copilot 技能" width="800"/>
...
1. **看看有哪些技能已經可用：**
   ...
   > 💡 **內建技能**：Copilot CLI 預設就有一些技能。例如，`customizing-copilot-cloud-agents-environment` 技能提供自訂 Copilot 雲端 agent 環境的指南。你不需要自己建立或安裝這些技能。執行 `/skills list` 就能看到有哪些可用。

2. **看看實際的技能檔案：** 參考我們提供的 [code-checklist SKILL.md](../.github/skills/code-checklist/SKILL.md) 來了解格式。它就是 YAML frontmatter 加上 markdown 指令。

3. **理解核心概念：** 技能是針對任務的指令，當你的提示詞符合技能描述時，Copilot 會*自動*載入。你不需要手動啟用，只要自然地詢問即可。

## 理解技能

Agent 技能是包含指令、腳本與資源的資料夾，Copilot 會在**與你的任務相關時自動載入**。Copilot 會讀取你的提示詞，檢查是否有技能符合，並自動套用相關指令。
...
> 💡 **關鍵洞見**：技能會根據你的提示詞是否符合技能描述**自動觸發**。只要自然地詢問，Copilot 就會在背後自動套用相關技能。你也可以直接呼叫技能，下一節會介紹。
...
你可以在**同一則訊息中呼叫多個技能**，而且技能的 slash 指令可以出現在提示詞的任何地方——不一定要在開頭。這在你想一次完成兩種檢查時很方便：
...
Copilot 會在同一回應中套用所有指定的技能，省下你多次發送訊息的麻煩。

> 💡 **小技巧**：技能 slash 指令可以放在句子的任何地方，依你覺得自然的位置即可。可以在開頭、中間或結尾。

> 📝 **技能與 Agent 呼叫方式的差異**：不要混淆技能與 Agent 的呼叫方式：
...
技能只是 GitHub Copilot 擴充模型的一部分。以下是它們與 Agent、MCP 伺服器的比較。

> *不用急著了解 MCP。我們會在[第 06 章](../06-mcp-servers/)詳細介紹。這裡先讓你了解技能在整體架構中的定位。*

<img src="assets/skills-agents-mcp-comparison.png" alt="比較圖，顯示 Agent、技能與 MCP 伺服器的差異，以及它們如何結合進你的工作流程" width="800"/>

| 功能 | 作用 | 適用時機 |
|---------|--------------|-------------|
| **Agent** | 改變 AI 的思考方式 | 需要跨多任務的專業知識時 |
| **技能** | 提供針對任務的指令 | 具體、可重複、步驟明確的任務 |
| **MCP** | 連接外部服務 | 需要從 API 取得即時資料時 |

Agent 適合廣泛專業知識，技能適合具體任務指令，MCP 則用於外部資料。Agent 在對話中可以同時使用一個或多個技能。例如，當你請 Agent 檢查程式碼時，它可能會自動套用 `security-audit` 技能和 `code-checklist` 技能。

> 📚 **深入了解**：完整技能格式與最佳實踐，請參考官方文件 [About Agent Skills](https://docs.github.com/copilot/concepts/agents/about-agent-skills)。

---

## 從手動提示到自動專業

在深入學習如何建立技能前，先看看*為什麼*值得學。當你看到一致性的提升後，「怎麼做」就會更有意義。
...
安裝好 `code-checklist` 技能後，只要自然詢問即可：
...
<img src="assets/skill-auto-discovery-flow.png" alt="技能自動觸發流程 - 4 步驟圖示 Copilot 如何自動將你的提示詞對應到正確技能" width="800"/>

*只要自然詢問，Copilot 會自動對應並套用正確技能。*
...
- [PASS] 沒有裸 except 子句
- [PASS] 沒有可變的預設參數
- [PASS] 檔案 I/O 使用 context manager
- [PASS] 函式皆小於 50 行
- [PASS] 變數與函式名稱符合 PEP 8

### Input Validation
- [FAIL] 使用者輸入未驗證 - add_book() 可接受任何 year 值
- [FAIL] 邊界情境未完整處理 - title/author 可為空字串
- [PASS] 錯誤訊息清楚且有幫助

### Testing
- [FAIL] 未找到對應的 pytest 測試

### Summary
有 3 項需在合併前處理
```

**差異**：你們團隊的標準每次都會自動套用，不用再重複輸入。
...
*Demo 輸出會有所不同。你的模型、工具與回應可能與這裡展示的不同。*
...
假設你們團隊有 10 點 PR 檢查清單。沒有技能時，每個開發者都要記住這 10 點，總有人會漏掉。用 `pr-review` 技能，整個團隊都能得到一致的審查：
...
Copilot 會自動載入你們的 `pr-review` 技能，檢查所有 10 點：
...
**威力**：每個團隊成員都能自動套用相同標準。新進人員不用背檢查清單，技能會自動處理。

---

# 建立自訂技能

<img src="assets/creating-managing-skills.png" alt="人類與機械手臂共同堆疊發光的樂高積木，象徵技能的建立與管理" width="800"/>

從 SKILL.md 檔案開始打造你的專屬技能。
...
每個技能都放在自己的資料夾，並有一個 `SKILL.md` 檔案。你也可以選擇加入腳本、範例或其他資源：
...
| `description` | **是** | 技能用途，以及 Copilot 何時應該使用 |
| `license` | 否 | 技能適用的授權條款 |
| `argument-hint` | 否 | 顯示給使用者的簡短提示，說明技能預期的參數（例如："file path or code snippet"） |

> 💡 **什麼是 `argument-hint`？** 當使用者直接呼叫技能（如 `/security-audit`）時，`argument-hint` 會作為佔位提示，告訴使用者接下來要輸入什麼——就像小型說明。例如設為 `argument-hint: "file path to review"`，就會提示使用者在技能名稱後輸入檔案路徑。

> 📖 **官方文件**：[About Agent Skills](https://docs.github.com/copilot/concepts/agents/about-agent-skills)

### 建立你的第一個技能

讓我們來打造一個檢查 OWASP Top 10 漏洞的安全性稽核技能：
...
SKILL.md 的 `description` 欄位非常重要！Copilot 就是靠這個決定是否載入你的技能：
...
> 💡 **小技巧**：請包含你平常會用來提問的關鍵字。如果你會說「security review」，那就把「security review」寫進 description。

### 技能與 Agent 的結合

技能與 Agent 可以搭配運作。Agent 提供專業知識，技能提供具體指令：
...
Copilot CLI 提供兩種技能管理方式。你可以在啟動 Copilot 前直接用終端機操作，也可以在 Copilot 互動會話中進行。

### 方式一：`copilot skill`（終端機指令）

`copilot skill` 子指令讓你直接在終端機管理技能，無需進入互動式 Copilot 會話。這對於腳本自動化、快速檢查或事前加裝技能很方便。
...
### 方式二：`/skills`（Copilot 會話內）

進入 Copilot 互動會話後，使用 `/skills`（或簡寫 `/skill`）即可管理技能，無需離開會話：
...
> 💡 **`/skill` 簡寫**：你可以輸入 `/skill` 取代 `/skills`——兩者等價。例如 `/skill list` 和 `/skills list` 效果相同。

> 💡 **請記住**：你不需要每次提示都「啟用」技能。安裝後，只要提示詞符合描述，技能就會**自動觸發**。這些指令是用來管理可用技能，而不是每次使用時都要手動啟用。
...
建立或編輯技能的 SKILL.md 檔案後，執行 `/skills reload` 即可套用變更，無需重啟 Copilot：
...
> 💡 **補充說明**：即使你用 `/compact` 摘要對話歷史，技能依然有效。壓縮後不需重新載入。
...
> 💡 **什麼是外掛？** 外掛是可安裝的套件，可以同時包含技能、Agent 與 MCP 伺服器設定。你可以把它們想像成 Copilot CLI 的「應用程式商店」擴充功能。
...
外掛可以同時包含多種功能。一個外掛可能內含相關的技能、Agent 與 MCP 伺服器設定，彼此協同運作。
...
從 GitHub 儲存庫安裝技能最簡單的方法是用 `gh skill install` 指令（需 [GitHub CLI v2.90.0+](https://github.blog/changelog/2026-04-16-manage-agent-skills-with-github-cli/)）：

```bash
# 互動式瀏覽並選擇 awesome-copilot 的技能
...
> ⚠️ **安裝前請審查**：安裝技能前務必閱讀其 `SKILL.md`。技能會控制 Copilot 的行為，惡意技能可能指示 Copilot 執行有害指令或意外修改程式碼。

---

# 練習

<img src="../assets/practice.png" alt="溫馨桌面擺設，螢幕顯示程式碼、檯燈、咖啡杯與耳機，準備進行實作練習" width="800"/>
...
這裡有兩個不同類型的技能範例。請依「建立你的第一個技能」的 `mkdir` + `cat` 流程操作，或直接複製貼上到正確位置。更多範例可見於 [.github/skills](../.github/skills)。
...
1. **Skill 創建挑戰**：建立一個 `quick-review` skill，執行三點檢查清單：
   - 裸露的 except 子句
   - 缺少型別註記
   - 變數名稱不明確
...
2. **Skill 效率比較**：計時自己手動撰寫詳細安全審查提示詞所需時間。然後只需詢問「Check for security issues in this file」，讓你的 security-audit skill 自動載入。這樣節省了多少時間？

3. **團隊 Skill 挑戰**：思考你們團隊的程式碼審查清單。能否將其編碼成一個 skill？寫下這個 skill 應該永遠檢查的三件事。

**自我檢查**：當你能解釋為什麼 `description` 欄位很重要時（這是 Copilot 判斷是否載入你的 skill 的依據），就代表你已經理解 skills 的運作。
...
前面的範例創建了 `pytest-gen` 和 `pr-review` 這兩個 skill。現在請練習創建一個完全不同類型的 skill：用來根據資料產生格式化輸出。

1. 列出你目前的 skills：執行 Copilot 並輸入 `/skills list`。你也可以用 `ls .github/skills/` 查看專案 skills，或用 `ls ~/.copilot/skills/` 查看個人 skills。
2. 在 `.github/skills/book-summary/SKILL.md` 建立一個 `book-summary` skill，能夠產生書籍收藏的格式化 markdown 摘要
3. 你的 skill 應具備：
   - 清楚的名稱與描述（description 對於比對至關重要！）
...
**起手範本**：建立 `.github/skills/book-summary/SKILL.md`：
...
# Skill 應該會根據 description 自動觸發
```

**如果沒有觸發：** 請嘗試 `/skills reload` 然後再問一次。

</details>

### 加分挑戰：Commit Message Skill

1. 建立一個 `commit-message` skill，能夠產生格式一致的 conventional commit 訊息
2. 將變更加入暫存區後，詢問：「Generate a commit message for my staged changes」
...
| 錯誤 | 會發生什麼事 | 修正方式 |
|------|--------------|----------|
| 檔案名稱不是 `SKILL.md` | Skill 不會被辨識 | 檔名必須完全為 `SKILL.md` |
| `description` 欄位過於模糊 | Skill 永遠不會自動載入 | Description 是主要的發現機制。請用明確的觸發關鍵字 |
| frontmatter 缺少 `name` 或 `description` | Skill 載入失敗 | YAML frontmatter 必須有這兩個欄位 |
| 資料夾位置錯誤 | 找不到 skill | 請用 `.github/skills/skill-name/`（專案）或 `~/.copilot/skills/skill-name/`（個人） |

### 疑難排解

**Skill 沒有被使用** - 如果 Copilot 沒有在預期時載入你的 skill：

1. **檢查 description**：是否與你的詢問方式相符？
   ```markdown
   # 不佳：太模糊
...
1. **檢查輸出格式**：如果你的 skill 有指定輸出格式（如 `[CRITICAL]` 標籤），請在回應中尋找這些標記
2. **直接詢問**：收到回應後問「Did you use any skills for that?」
3. **比較有無 skill 差異**：用 `--no-custom-instructions` 試試同一個提示：
   ```bash
   # 有 skills
   ...
   ```
4. **檢查特定檢查項目**：如果你的 skill 包含特定檢查（如「函式超過 50 行」），請確認這些內容有出現在輸出中
...
1. **Skills 會自動載入**：當你的提示詞與 skill 的 description 相符時，Copilot 會自動載入
2. **可直接呼叫**：你也可以用 `/skill-name` slash 指令直接呼叫 skill
...
5. **Description 是關鍵**：請用自然詢問方式撰寫 description，讓比對更精準
6. **管理 skills 的兩種方式**：可在終端機用 `copilot skill`，或在會話中用 `/skills`（快捷鍵：`/skill`）

> 📋 **快速參考**：完整指令與快捷鍵請參見 [GitHub Copilot CLI command reference](https://docs.github.com/en/copilot/reference/cli-command-reference)。
