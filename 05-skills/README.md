...
> **如果 Copilot 能自動套用你們團隊的最佳實踐，而不需要你每次都重新解釋，會怎麼樣？**

在本章中，你將學習關於 Agent 技能（Skills）：Copilot 會在與你的任務相關時自動載入的指令資料夾。Agent 會改變 Copilot *思考的方式*，而技能則教 Copilot *完成任務的具體方法*。你將建立一個安全稽核技能，讓 Copilot 在你詢問安全性時自動套用，打造團隊標準的審查規範，確保程式碼品質一致，並學會技能如何在 Copilot CLI、VS Code 及 GitHub Copilot 雲端代理人中運作。
...
1. **查看現有技能：**
   ...
   > 💡 **內建技能**：Copilot CLI 預設就有一些技能。例如，`customizing-copilot-cloud-agents-environment` 技能提供自訂 Copilot 雲端代理人環境的指南。你不需要額外建立或安裝，直接執行 `/skills list` 就能看到有哪些技能可用。

2. **看看實際的技能檔案：** 參考我們提供的 [code-checklist SKILL.md](../.github/skills/code-checklist/SKILL.md) 來了解格式。它其實就是 YAML frontmatter 加上 markdown 指令。

3. **理解核心概念：** 技能是 Copilot 會在你的提示詞符合技能描述時*自動*載入的任務專用指令。你不需要手動啟用，只要自然地提問即可。
...
> 💡 **關鍵洞見**：技能會根據你的提示詞是否符合技能描述**自動觸發**。只要自然提問，Copilot 就會在背後自動套用相關技能。你也可以直接呼叫技能，下一節會介紹。

> 🧰 **現成範本**：參考 [.github/skills](../.github/skills/) 資料夾，裡面有可直接複製貼上的簡單技能。
...
你可以在**同一則訊息中呼叫多個技能**，而且技能斜線指令可以出現在提示詞的任何地方——不一定要在開頭。這很適合一次做兩種檢查：
...
> 💡 **提示**：把技能斜線指令放在你覺得最自然的位置。可以在訊息的開頭、中間或結尾。

> 📝 **技能 vs Agent 呼叫**：不要把技能呼叫和 Agent 呼叫搞混：
...
| **技能** | 提供任務專用指令 | 具體、可重複、有明確步驟的任務 |
| **MCP** | 連接外部服務 | 需要從 API 取得即時資料 |

Agent 適合廣泛專業知識，技能適合具體任務指令，MCP 則用於外部資料。Agent 在對話中可以同時使用一個或多個技能。例如，當你請 Agent 檢查程式碼時，它可能會自動套用 `security-audit` 和 `code-checklist` 技能。
...
在學習如何建立技能之前，先來看看*為什麼*值得學。當你看到一致性的提升後，"怎麼做" 就會更有意義。
...
假設你們團隊有 10 點 PR 檢查清單。沒有技能時，每個開發者都要記住這 10 點，總有人會漏掉。有了 `pr-review` 技能，整個團隊都能得到一致的審查：
...
**威力**：每位團隊成員都能自動套用相同標準。新進人員也不用背清單，技能會自動處理。
...
技能儲存在 `.github/skills/`（專案專用）或 `~/.copilot/skills/`（使用者層級）。
...
| 位置 | 範圍 |
|----------|-------|
| `.github/skills/` | 專案專用（透過 git 與團隊共用） |
| `~/.copilot/skills/` | 使用者專用（你的個人技能） |

### 技能結構

每個技能都有自己的資料夾和 `SKILL.md` 檔案。你也可以選擇加入腳本、範例或其他資源：
...
> 💡 **提示**：資料夾名稱應與 SKILL.md frontmatter 的 `name` 欄位一致（小寫、用連字號）。
...
來打造一個檢查 OWASP Top 10 漏洞的安全稽核技能：
...
SKILL.md 的 `description` 欄位非常重要！Copilot 就是靠這個判斷是否要載入你的技能：
...
> 💡 **提示**：請包含你平常提問時會用到的關鍵字。如果你會說「security review」，就把「security review」寫進描述裡。

### 技能與 Agent 結合

技能和 Agent 可以搭配運作。Agent 提供專業知識，技能則給予具體指令：
...
| `/skills add <name>` | 啟用技能（從儲存庫或市集安裝） |
| `/skills remove <name>` | 停用或解除安裝技能 |
| `/skills reload` | 編輯 SKILL.md 後重新載入技能 |

> 💡 **請記住**：你不需要每次提示都「啟用」技能。只要安裝好，當你的提示詞符合描述時，技能就會**自動觸發**。這些指令是用來管理技能可用性，不是用來直接使用技能。
...
> 💡 **小知識**：即使你用 `/compact` 指令壓縮對話記錄，技能依然有效。壓縮後不需重新載入技能。
...
> ⚠️ **安裝前請審查**：安裝技能前，務必閱讀該技能的 `SKILL.md`。技能會控制 Copilot 的行為，惡意技能可能會指示 Copilot 執行有害命令或意外修改程式碼。
...
2. **技能比較**：計時你手動輸入詳細安全審查提示詞所需時間。然後只問「Check for security issues in this file」，讓 security-audit 技能自動載入。技能幫你省下多少時間？

3. **團隊技能挑戰**：思考你們團隊的程式碼審查清單。能否將它寫成技能？寫下技能應該永遠檢查的三件事。

**自我檢查**：你能解釋 `description` 欄位為何重要（Copilot 就是靠它判斷是否載入技能），就代表你已經理解技能了。
...
上面的範例建立了 `pytest-gen` 和 `pr-review` 技能。現在請練習建立一個完全不同類型的技能：用來從資料產生格式化輸出的技能。1. 列出你目前的技能：執行 Copilot 並傳入 `/skills list`。你也可以使用 `ls .github/skills/` 來查看專案技能，或 `ls ~/.copilot/skills/` 查看個人技能。
2. 在 `.github/skills/book-summary/SKILL.md` 建立一個 `book-summary` 技能，能夠產生書籍收藏的格式化 markdown 摘要
3. 你的技能應包含：
   - 清楚的名稱與描述（描述對於自動比對至關重要！）
   - 明確的格式規則（例如：使用 markdown 表格，欄位有 title、author、year、read status）
   - 輸出慣例（例如：已讀狀態用 ✅/❌，依年份排序）
...
**成功標準**：你有一個可用的 `book-summary` 技能，當你詢問書籍收藏時 Copilot 能自動套用。
...
| 錯誤 | 會發生什麼事 | 修正方式 |
|------|--------------|----------|
| 檔名不是 `SKILL.md` | 技能不會被辨識 | 檔案名稱必須完全是 `SKILL.md` |
| `description` 欄位太模糊 | 技能永遠不會自動載入 | 描述是主要的發現機制，請使用明確的觸發關鍵字 |
| frontmatter 缺少 `name` 或 `description` | 技能載入失敗 | 在 YAML frontmatter 裡加上這兩個欄位 |
| 資料夾位置錯誤 | 找不到技能 | 使用 `.github/skills/skill-name/`（專案）或 `~/.copilot/skills/skill-name/`（個人） |

### 疑難排解

**技能未被使用**－如果 Copilot 沒有如預期使用你的技能：
...
1. **檢查輸出格式**：如果你的技能有指定輸出格式（如 `[CRITICAL]` 標籤），請在回應中尋找
2. **直接詢問**：收到回應後問「Did you use any skills for that?」
3. **有無技能比較**：用 `--no-custom-instructions` 比較同一提示的結果：
   ```bash
   # 有技能
   ...
   ```
4. **檢查特定檢查項目**：如果你的技能有特定檢查（如「函式超過 50 行」），看看輸出中是否有出現
...
1. **技能是自動的**：當你的提示詞符合技能描述時，Copilot 會自動載入
...
技能讓 Copilot 能自動載入指令擴充能力。但如果要連接外部服務呢？這就是 MCP 的用武之地。
