...
> 📖 **先決條件**：請先完成 [第 02 章：情境與對話](../02-context-conversations/README.md) 再閱讀本附錄。

本附錄介紹兩項進階情境功能：圖片操作與跨多個目錄的權限管理。
...
**3. 錯誤分析**
```bash
> @error-screenshot.png 這個錯誤代表什麼？要怎麼修正？
```
...
> 標頭在截圖裡看起來不對。程式碼裡是什麼原因？
```

### 圖片使用小技巧

- **裁剪截圖**，只保留相關區域（節省情境 token）
- **提高對比度**，讓要分析的 UI 元素更明顯
- **必要時加註解**——上傳前圈出或標示問題區域
- **一個概念用一張圖**——多張圖片可以，但要聚焦

---

## 權限模式

預設情況下，Copilot 只能存取你目前目錄的檔案。若要存取其他位置的檔案，需額外授權。
...
**[← 回到第 02 章](../02-context-conversations/README.md)** | **[返回附錄列表](README.md)**
