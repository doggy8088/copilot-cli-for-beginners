...
以下每個工作流程都是獨立的。你可以選擇符合你目前需求的流程，或全部依序練習。

---

## 選擇你的冒險

本章涵蓋開發者常用的五種工作流程。**但你不需要一次全部讀完！** 每個流程都獨立收納在下方的可展開區塊。選擇你需要、且最適合目前專案的流程即可。你隨時可以回來探索其他流程。
...
| 寫程式前先做研究 | [快速提示：規劃或寫程式前先研究](#quick-tip-research-before-you-plan-or-code) |
| 看完整的錯誤修正流程 | [整合應用](#putting-it-all-together-bug-fix-workflow) |
...
使用多輪對話深入探討。先做廣泛審查，再提出後續問題，無需重新開始。
...
請 Copilot CLI 以特定格式輸出（例如可貼到 issue 的嚴重性分類 markdown 清單）。
...
在使用 `/review` 指令前，你需要了解 git 中的兩種變更：
...
`/review` 指令會呼叫內建的**程式碼審查 agent**，專為分析已暫存與未暫存變更設計，能提供高品質、重點明確的回饋。使用斜線指令可觸發專屬內建 agent，無需手動撰寫提示。
...
從簡單的改善開始。可在書籍應用程式上嘗試以下範例。每個提示都用 `@` 檔案參照搭配明確的重構指示，讓 Copilot CLI 知道要改什麼。
...
> 💡 **重構新手？** 先從加上型別註記或改善變數名稱等簡單請求開始，再挑戰複雜轉換。
...
先描述發生了什麼問題。以下是你可以在 buggy 書籍應用程式上嘗試的常見除錯模式。每個提示都用 `@` 檔案參照搭配明確的症狀描述，讓 Copilot CLI 能定位並診斷錯誤。
...
這就是情境感知除錯的威力。請用 buggy 書籍應用程式嘗試這個情境。用 `@` 提供整個檔案，只描述使用者回報的症狀。Copilot CLI 會追查根本原因，甚至可能發現附近的其他錯誤。
...
**為什麼這很重要**：Copilot CLI 讀取整個檔案，理解你的錯誤報告情境，並給你明確修正與清楚解釋。

> 💡 **加分**：因為 Copilot CLI 會分析整個檔案，常常能發現你沒問到的*其他*問題。例如修正作者搜尋時，可能也會發現 `find_book_by_title` 的大小寫問題！

### 真實世界安全側欄

除錯自己的程式很重要，理解生產環境應用程式的安全漏洞更是關鍵。請嘗試這個範例：將 Copilot CLI 指向一個陌生檔案，請它稽核安全性問題。
...
將堆疊追蹤直接貼到提示中，搭配 `@` 檔案參照，讓 Copilot CLI 能將錯誤對應到原始碼。
...
描述確切的輸入與觀察到的輸出，讓 Copilot CLI 能以具體、可重現的測試案例進行推理。
...
同時提供資料檔與讀取它的程式碼，讓 Copilot CLI 在建議錯誤處理改善時能掌握全貌。
...
看看當你請 Copilot CLI 產生全面測試時會發生什麼！這個提示用條列清單搭配 `@` 檔案參照，指引 Copilot CLI 產生完整測試覆蓋：
...
**結果**：30 秒內你就能得到本來要花一小時思考與撰寫的邊界測試。
...
用白話問題詢問你的工具鏈，Copilot CLI 會幫你產生正確的 shell 指令。
...
列出進階或棘手情境，讓 Copilot CLI 不只測試快樂路徑。
...
> **先試試這個：** `copilot -p "Generate a conventional commit message for: $(git diff --staged)"` — 先將變更暫存，再執行這行，看看 Copilot CLI 幫你寫提交訊息。

這個範例用 `-p` 行內提示旗標搭配 shell 指令替換，將 `git diff` 輸出直接傳給 Copilot CLI，一次產生提交訊息。`$(...)` 語法會執行括號內指令，並將結果插入外層指令。
...
將 `git show` 輸出導入 `-p` 提示，取得上一筆提交的白話摘要。
...
在 Copilot CLI 互動模式下工作時，可以用 `/pr` 指令操作 pull request。用 `/pr` 檢視 PR、建立新 PR、修正現有 PR，或讓 Copilot CLI 根據分支狀態自動決定。
...
`/delegate` 指令會將工作交給 GitHub 上的 Copilot 程式碼 agent。用 `/delegate` 斜線指令（或 & 快捷鍵）將明確任務交給背景 agent 處理。
...
`/diff` 指令會顯示你本次工作階段所有變更。用這個斜線指令，在提交前檢視 Copilot CLI 修改過的所有檔案差異。
...
當你需要調查函式庫、了解最佳實踐，或探索陌生主題時，可用 `/research` 先進行深入研究，再開始寫程式：
...
Copilot 會搜尋 GitHub 儲存庫與網路資源，然後回傳帶有參考資料的摘要。這在你要開始新功能、想先做出明智決策時特別有用。你也可以用 `/share` 分享結果。

> 💡 **提示**：`/research` 很適合在 `/plan` 之前使用。先研究做法，再規劃實作。
...
# 2. 偵錯問題（在同一個工作階段繼續）
> 根據分析，請顯示 find_by_author 函式並解釋問題
...
<img src="../images/practice.png" alt="溫馨桌面擺設，螢幕顯示程式碼、檯燈、咖啡杯與耳機，準備動手練習" width="800"/>
...
3. **提交訊息挑戰**：對書籍應用程式的任一檔案做些微小修改，暫存（`git add .`），然後執行：
   ```bash
   copilot -p "Generate a conventional commit message for: $(git diff --staged)"
   ```
   這個訊息比你自己快速寫的還要好嗎？

**自我檢查**：當你能解釋為什麼「debug this bug」比「find bugs」更強大時（情境很重要！），就代表你已經理解開發工作流程了！
...
實作範例聚焦於 `find_book_by_title` 和程式碼審查。現在請你在 `book-app-project` 的其他函式上練習相同的工作流程技能：
...
**小技巧：** 改善完 `remove_book()` 後，試著問 Copilot CLI：「Are there any other functions in this file that could benefit from the same improvements?」。它可能會建議你對 `find_book_by_title()` 或 `find_by_author()` 做類似的優化。
...
這個練習會教你如何使用 GitHub Copilot CLI，在建立 Node.js 計算機應用程式的過程中，從終端機建立議題、產生程式碼並協作。你會安裝 CLI、使用範本與 Agent，並練習以指令列為主的反覆開發。
...
| 錯誤 | 會發生什麼 | 修正方式 |
|---------|--------------|-----|
| 使用模糊的提示詞如 "Review this code" | 只得到泛泛的回饋，錯過具體問題 | 要具體：如 "Review for SQL injection, XSS, and auth issues" |
| 沒用 `/review` 進行程式碼審查 | 錯過最佳化的程式碼審查 Agent | 使用 `/review`，它會產生高訊噪比的審查結果 |
| 只說 "find bugs" 沒有情境 | Copilot CLI 不知道你遇到什麼錯誤 | 描述症狀："Users report X happens when Y" |
...
1. **程式碼審查**：使用具體提示詞讓審查更全面
2. **重構**：先產生測試再重構更安全
3. **偵錯**：同時給 Copilot CLI 錯誤現象和程式碼效果最好
4. **測試產生**：要涵蓋邊界情境與錯誤狀況
5. **Git 整合**：自動產生提交訊息與 PR 描述

> 📋 **快速參考**：[GitHub Copilot CLI 指令參考](https://docs.github.com/en/copilot/reference/cli-command-reference) 可查詢完整指令與快捷方式。

---

## ✅ 檢查點：你已掌握核心技能

**恭喜！** 你現在已具備 GitHub Copilot CLI 的所有核心生產力技能：

| 技能 | 章節 | 你現在可以... |
|-------|---------|----------------|
| 基本指令 | Ch 01 | 使用互動模式、規劃模式、程式化模式（-p）與斜線指令 |
| 情境 | Ch 02 | 以 `@` 參照檔案、管理對話、理解 context window |
...
使用 GitHub Copilot CLI 沒有唯一「正確」方式。以下是幾個建立自己習慣的建議：

> 📚 **官方文件**：[Copilot CLI 最佳實踐](https://docs.github.com/copilot/how-tos/copilot-cli/cli-best-practices)——GitHub 官方推薦的工作流程與技巧。

- **遇到複雜任務先用 `/plan`**。先規劃再執行，計畫好結果更好。
- **把有效的提示詞存下來。** 當 Copilot CLI 出錯時，記下原因。久而久之，這會成為你的個人秘笈。
- **勇於嘗試。** 有些開發者喜歡長提示，有些則偏好短提示加追問。多試幾種，找出最順手的方式。
...
**建議**：先用這些核心工作流程一週，再依需求回來學第 04-06 章。
